...
const UI = {
    ...
    closeInstructorModalBtn: document.getElementById(
        'btn-close-instructor-modal'
    ),
    ...
    openInstructorModalBtn: document.querySelector(
        '.btn-open-instructor-modal'
    ),
...
const AppUtils = {
    ...
    getDirectImageLink: (url) => {
        // Converte links do Drive e GitHub para links diretos de imagem
        if (!url) return url;
...
const ModalLogic = {
    // Lóigica para abrir e fechar o modal, incluindo a restauração do conteúdo original do formulário se necessário
    open: (modalEl = UI.modal) => {
    ...
    close: (modalEl = UI.modal) => {
        // Fecha o modal e permite o scroll do body novamente
        ...
        setTimeout(() => {
            ...
            if (modalEl.dataset.originalContent) {
                ...
                if (modalEl === UI.modal) {
                    ...
                    UI.instructorForm =
                        document.getElementById('instructor-form');
                    UI.instructorForm.addEventListener(
                        'submit',
                        Handlers.handleInstructorSubmit
                    );
                }
...
const Handlers = {
    ...
    async handleInstructorSubmit(e) {
        ...
        const formData = {
            ...
            support: document.querySelector('input[name="support"]:checked')
                ?.value
        };
...
UI.openModalBtns.forEach((btn) =>
    btn.addEventListener('click', () => ModalLogic.open(UI.modal))
);
UI.closeModalBtn.addEventListener('click', () => ModalLogic.close(UI.modal));

if (UI.openInstructorModalBtn) {
    UI.openInstructorModalBtn.addEventListener('click', () =>
        ModalLogic.open(UI.modalInstructor)
    );
}
if (UI.closeInstructorModalBtn) {
    UI.closeInstructorModalBtn.addEventListener('click', () =>
        ModalLogic.close(UI.modalInstructor)
    );
}
...
if (UI.closeProjectModalBtn) {
    UI.closeProjectModalBtn.addEventListener('click', () =>
        ModalLogic.close(UI.modalProject)
    );
}
