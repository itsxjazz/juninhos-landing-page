...
var _getViewportDimension = function _getViewportDimension2(dimensionProperty) {
	return (
		(dimensionProperty === "Height" ? _100vh : _win2["inner" + dimensionProperty]) ||
...
var _callback = function _callback2(self, func, extraParam) {
	if (self.enabled) {
		var result = self._ctx ? self._ctx.add(() => func(self, extraParam)) : func(self, extraParam)
...
var _getClosestLabel = function _getClosestLabel2(animation) {
	return value => gsap2.utils.snap(_getLabelRatioArray(animation), value)
}
var _snapDirectional = function _snapDirectional2(snapIncrementOrArray) {
	var snap = gsap2.utils.snap(snapIncrementOrArray),
		a = Array.isArray(snapIncrementOrArray) && snapIncrementOrArray.slice(0).sort((a2, b) => a2 - b)
	return a
...
var _onResize = function _onResize2(force) {
	...
	;(force === true ||
		(!_refreshing &&
			...
			(!_ignoreMobileResize ||
				_baseScreenWidth !== _win2.innerWidth ||
				Math.abs(_win2.innerHeight - _baseScreenHeight) > _win2.innerHeight * 0.25))) &&
...
var _recordScrollPositions = function _recordScrollPositions2() {
	return _scrollers.forEach(obj => _isFunction(obj) && ++obj.cacheID && (obj.rec = obj()))
}
...
var _parsePosition = function _parsePosition2(
	...
	if (marker) {
		...
		_positionMarker(
			...
			(isStart && position > 20) ||
				(!isStart &&
					(useFixedPosition ? Math.max(_body2[p1], _docEl2[p1]) : marker.parentNode[p1]) <=
						position + 1),
		)
		if (useFixedPosition) {
...
var ScrollTrigger2 = (() => {
	...
	_proto.init = function init(vars, animation) {
		...
		if (snap) {
			...
			_scrollers.forEach(
				o =>
					_isFunction(o) &&
					o.target === (isViewport ? _doc2.scrollingElement || _docEl2 : scroller) &&
					(o.smooth = false),
			)
		...
		if (markers) {
			...
			containerAnimation &&
				(caMarkerSetter = gsap2.quickSetter([markerStart, markerEnd], direction.a, _px))
			if (
				!useFixedPosition &&
		...
		self.refresh = (soft, force, position, pinOffset) => {
			...
			if (pin && !(_clampingMax && self.end >= _maxScroll(scroller, direction))) {
				...
				if (!max && end > 1) {
					...
					if (
						isViewport &&
						_getComputedStyle(_body2)["overflow" + direction.a.toUpperCase()] !== "scroll"
					) {
	...
	ScrollTrigger3.enable = function enable() {
		...
		if (gsap2) {
			...
			if (_body2) {
				...
				_root2 = [_win2, _doc2, _docEl2, _body2]
				if (gsap2.matchMedia) {
					ScrollTrigger3.matchMedia = vars => {
					...
					gsap2.addEventListener("matchMediaInit", () => {
					...
					gsap2.matchMedia().add("(orientation: portrait)", () => {
						_setBaseDimensions()
	...
	ScrollTrigger3.config = function config(vars) {
		...
		"ignoreMobileResize" in vars &&
			(_ignoreMobileResize = ScrollTrigger3.isTouch === 1 && vars.ignoreMobileResize)
...
ScrollTrigger2.saveStyles = targets =>
	targets
...
ScrollTrigger2.refresh = safe =>
	safe ? _onResize(true) : (_coreInitted2 || ScrollTrigger2.register()) && _refreshAll(true)
ScrollTrigger2.update = force => ++_scrollers.cache && _updateAll(force === true ? 2 : 0)
ScrollTrigger2.clearScrollMemory = _clearScrollMemory
ScrollTrigger2.maxScroll = (element, horizontal) =>
	_maxScroll(element, horizontal ? _horizontal : _vertical)
ScrollTrigger2.getScrollFunc = (element, horizontal) =>
...
ScrollTrigger2.batch = (targets, vars) => {
	...
	if (_isFunction(batchMax)) {
		batchMax = batchMax()
		_addListener3(ScrollTrigger2, "refresh", () => (batchMax = vars.batchMax()))
	}
	_toArray(targets).forEach(target => {
		var config = {}
		for (p in varsCopy) {
...
ScrollTrigger2.sort = func => {
	if (_isFunction(func)) {
		return _triggers.sort(func)
	}
	var scroll = _win2.pageYOffset || 0
	ScrollTrigger2.getAll().forEach(
		t =>
			(t._sortY = t.trigger
