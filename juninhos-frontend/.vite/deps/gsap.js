...
var _isTypedArray = (typeof ArrayBuffer === "function" && ArrayBuffer.isView) || (() => {})
...
var wrap = function wrap2(min, max, value) {
	...
	return _isArray(min)
		...
		: _conditionalReturn(value, value2 => ((range + ((value2 - min) % range)) % range) + min)
}
var wrapYoyo = function wrapYoyo2(min, max, value) {
	...
	return _isArray(min)
		...
		: _conditionalReturn(value, value2 => {
				value2 = (total + ((value2 - min) % total)) % total || 0
				return min + (value2 > range ? total - value2 : value2)
			})
}
var _replaceRandom = function _replaceRandom2(s) {
	return s.replace(_randomExp, match => {
...
var mapRange = function mapRange2(inMin, inMax, outMin, outMax, value) {
	...
	return _conditionalReturn(
		value,
		value2 => outMin + (((value2 - inMin) / inRange) * outRange || 0),
	)
}
var interpolate = function interpolate2(start, end, progress, mutate) {
	var func = isNaN(start + end) ? 0 : p2 => (1 - p2) * start + p2 * end
	if (!func) {
...
var _configElastic = function _configElastic2(type, amplitude, period) {
	...
	p2 = _2PI / p2
	ease.config = (amplitude2, period2) => _configElastic2(type, amplitude2, period2)
	return ease
}
		...
		ease =
			type === "out"
				...
				: type === "in"
					? p => 1 - easeOut(1 - p)
					: _easeInOutFromOut(easeOut)
	ease.config = overshoot2 => _configBack2(type, overshoot2)
	return ease
}
			...
			while (child) {
				...
				if (start < 0 && child._ts) {
					...
					if ((!parent && !this._dp) || (parent && parent.smoothChildTiming)) {
						this._start += _roundPrecise(start / this._ts)
						this._time -= start
						this._tTime -= start
					}
					this.shiftChildren(-start, false, -Infinity)
					prevStart = 0
				}
...
var Context = (() => {
	...
	_proto5.add = function add(name, func, scope) {
		...
		return name === _isFunction
			? f(self, func2 => self.add(null, func2))
			: name
				? (self[name] = f)
				: f
	}
...
var MatchMedia = (() => {
	...
	_proto6.add = function add(conditions, func, scope) {
		...
		active && func(context3, f => context3.add(null, f))
		return this
	}
...
var _gsap = {
	registerPlugin: function registerPlugin() {
		...
		args.forEach(config3 => _createPlugin(config3))
	},
	...
	registerEffect: function registerEffect(_ref3) {
		...
		_effects[name] = (targets, vars, tl) =>
			effect(toArray(targets), _setDefaults(vars || {}, defaults2), tl)
		if (extendTimeline) {
...
var _saveStyle = function _saveStyle2(property, isNotCSS) {
	...
	if (property in _transformProps && style) {
		...
		if (property !== "transform") {
			...
			return _propertyAliases.transform.split(",").forEach(p => _saveStyle2.call(this, p, isNotCSS))
		}
