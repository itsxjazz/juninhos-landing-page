...
export function ResetPasswordPage() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!token) {
      return setFeedback({ msg: 'Token de redefinição ausente na URL.', type: 'error' });
    }
    ...
    if (password.length < 6) {
      return setFeedback({ msg: 'Senha deve ter no mínimo 6 caracteres.', type: 'error' });
    }
    if (password !== confirm) {
    ...
    try {
      ...
      setFeedback({ msg: 'Senha redefinida com sucesso. Redirecionando…', type: 'success' });
  ...
  return (
    <main className="container py-32 min-h-screen flex flex-col items-center">
      ...
      <form onSubmit={handleSubmit} className="card-item !p-7 flex flex-col gap-4 w-full max-w-[460px]">
        <div className="form-group !mb-0">
          <label htmlFor="new-password">Nova senha</label>
          <input type="password" id="new-password" name="password" required placeholder="Min 6 caracteres" />
        </div>
        <div className="form-group !mb-0">
          <label htmlFor="confirm-password">Confirmar nova senha</label>
          <input type="password" id="confirm-password" name="confirm" required placeholder="Repita a senha" />
        </div>
      ...
      {feedback && (
        <div className={`form-feedback ${feedback.type} mt-4 max-w-[460px] w-full`}>{feedback.msg}</div>
      )}

      <p className="text-center text-text-muted text-sm mt-6">
        <Link to="/login" className="hover:text-primary transition-colors">← Voltar ao login</Link>
      </p>
    </main>
  );
}
