...
export function LoginPage() {
	...
	const handleLogin = async (e: FormEvent<HTMLFormElement>) => {
		...
		setFeedback(null)
		try {
			const res = await ApiService.postAuth<AuthSuccess>(CONFIG.ENDPOINTS.LOGIN, {
				email,
				password,
			})
	...
	const handleRegister = async (e: FormEvent<HTMLFormElement>) => {
		...
		if (!EMAIL_REGEX.test(email))
			return setFeedback({ msg: "Formato de e-mail inválido.", type: "error" })
		if (strength < 4)
			return setFeedback({
				msg: "Senha fraca. Precisa de letra maiúscula, número e símbolo (min 8).",
				type: "error",
			})
		...
		setFeedback(null)
		try {
			await ApiService.postAuth(CONFIG.ENDPOINTS.REGISTER, {
				name,
				email,
				password,
			})
			setFeedback({
				msg: "Conta criada! Faça login pra continuar.",
				type: "success",
			})
			setTab("login")
			setRegisterPassword("")
	...
	const handleForgot = async (e: FormEvent<HTMLFormElement>) => {
		...
		setFeedback(null)
		try {
			const res = await ApiService.postAuth(CONFIG.ENDPOINTS.FORGOT_PASSWORD, {
				email,
			})
		...
		<div className="min-h-screen relative overflow-hidden -mt-20 pt-20">
			...
			<main className="relative z-10 container grid lg:grid-cols-[1.1fr_1fr] gap-12 lg:gap-20 py-10 lg:py-16 items-start">
				...
				<section className="w-full max-w-[460px] mx-auto lg:ml-auto">
					...
					{feedback && <div className={`form-feedback ${feedback.type} mt-4`}>{feedback.msg}</div>}
