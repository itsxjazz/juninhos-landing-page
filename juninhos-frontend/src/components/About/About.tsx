...
const DISCORD_INVITE_CODE = import.meta.env.VITE_DISCORD_INVITE_CODE as
  | string
  | undefined;
...
export function About() {
  ...
  const roundedMemberCount =
    Math.floor(memberCount !== null ? memberCount / 100 : 0) * 100;
  const formattedMemberCount =
    roundedMemberCount >= 1000
      ? `${roundedMemberCount / 1000}K`
      : roundedMemberCount.toString();
  const membersDisplay =
    memberCount !== null ? `+${formattedMemberCount}` : "+300";
  const membersLabel =
    discord.total !== null ? "Membros na Comunidade" : "Membros na Comunidade";
    ...
    <section id="about" className="about-section container">
      <header className="section-header-left">
        ...
        <p className="section-subtitle">
          A comunidade colaborativa onde teoria vira código.
        </p>
      </header>
      ...
      <div className="grid md:grid-cols-2 gap-x-10 gap-y-20">
        {ABOUT_BLOCKS.map((block) => (
          ...
          >
            ...
            <p className="text-text-muted leading-relaxed text-[1rem]">
              {block.body}
            </p>
          </div>
        ))}
      ...
      <div className="stats-grid mt-32">
        <div className="stat-item card-item">
          <strong className={discord.loading ? "opacity-60" : ""}>
            {membersDisplay}
          </strong>
